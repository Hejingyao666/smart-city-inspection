...
def log_train_metrics(
    ...
    *,
    ...
    log.info(thin)
    for k_disp, k_data in [("预处理", "preprocess"), ("推理", "inference"), ("损失计算", "loss"), ("后处理", "postprocess"), ("总计", "total")]:
    ...
    log.info(thin)
    log.info(f"{pad_to_width('Fitness 分数', key_width)}: {metrics.overall.get('fitness', math.nan):.4f}")

    metric_fields = _METRIC_FIELDS_BY_TASK.get(metrics.task, [])
    if metric_fields:
        for raw_key, display in metric_fields:
            log.info(f"{pad_to_width(display, key_width)}: {metrics.overall.get(raw_key, math.nan):.4f}")
    else:
        log.info(f"(task='{metrics.task}' 不在 _METRIC_FIELDS_BY_TASK, 打印 results_dict 全量)")
