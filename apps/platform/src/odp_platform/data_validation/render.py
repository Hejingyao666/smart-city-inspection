...
def render_to_markdown(report: ValidationReport, output_path: Path) -> None:
    """将 ValidationReport 渲染成 Markdown 文件。
    ...
    lines.append(f"**task**: `{report.snapshot.task_type}`  \n")
    ...
    # 检查项一览
    lines.append("## 检查项一览\n")
    lines.append("| check name | severity | summary |\n")
    lines.append("|------------|----------|---------|\n")
    for r in report.results:
        lines.append(f"| {r.name} | {r.severity} | {r.summary} |\n")
    lines.append("\n---\n")

    # 数据集摘要
    lines.append("## 数据集摘要\n")
    lines.append("| split | image_count | annotated_count | total_instances |\n")
    lines.append("|-------|-------------|-----------------|-----------------|\n")
    ...
    # 类别列表（前30）
    class_names = report.snapshot.class_names
    if class_names:
        lines.append(f"## 类别 (共 {len(class_names)} 个)\n")
        preview = ", ".join(class_names[:30])
        if len(class_names) > 30:
            preview += f" ... 等 {len(class_names)} 个"
