...
class ThreadedPipeline:
    ...
    def run(self, stats) -> bool:
        ...
        # ★ 权威 batch 日志: 实时源低延迟优先, batch 恒为 1 (攒批=攒延迟, 对直播无意义).
        #   之前 service 那条 "batch=16" 会让人误以为摄像头在用 16, 这里把真相打出来.
        if live and self.batch_size > 1:
            logger.warning(
                "实时源(摄像头/RTSP) batch 强制为 1 (请求的 %d 被忽略): "
                "攒批会引入约 %d 帧的排队延迟, 与实时性冲突. "
                "提速请走 FP16 / TensorRT / 降 imgsz, 而不是加 batch.",
                self.batch_size, self.batch_size,
            )
        logger.info("实际生效 batch=%d (源类型: %s)", eff_batch, "实时流" if live else "文件")
