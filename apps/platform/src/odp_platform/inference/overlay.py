...
帧率口径(写论文/快照都用得上):
  ...
  - infer(推理)   : 模型整帧吞吐(预处理+推理+后处理 之和的倒数), 取自 ultralytics
                    result.speed; 是"纯模型一帧多快", 不含读帧/绘制/显示
...
class Metrics:
    ...
    def add_speed(self, speed: dict | None) -> None:
        ...
        ★ infer 口径修正: 原来只喂 inference 一项, 算出的"推理 FPS"只反映纯 GPU
          前向, 不含预处理/后处理 —— 数值虚高且会被误读成系统吞吐(实测摄像头
          "推理 100" 而 loop 只有 75). 现改为喂【预处理+推理+后处理】之和, 即
          model() 跑一帧的完整成本, 让"推理 FPS"= 模型整帧吞吐上限(仍不含读帧/绘制/显示).
          它与 loop 的差距 = 管道 + Python 包装税; 两者越接近, 说明模型本身越是瓶颈.
          各阶段拆分仍由 speed_avg_ms() / 日志的 speed_ms 行单独给出, 纯推理信息不丢.
        """
        if not speed:
            return
        pp = float(speed.get("preprocess", 0.0))
        ...
        total = pp + inf + post
        if total > 0:
            self.infer.update(total)   # ★ 整帧成本(pre+inf+post), 不再是 inference-only
