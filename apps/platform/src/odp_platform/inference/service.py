...
_PREDICT_KEYS: tuple[str, ...] = (
    ...
    "half",   # ★ FP16 推理开关: 之前漏在白名单外, 导致 yaml/CLI 写的 half=True 永远传不进 model().
              #   注: config 里的 amp 是【训练】用的混合精度, 对 predict 无效, 推理要 FP16 必须靠 half.
)
    ...
    def predict(
        ...
        *,
        ...
        try:
            ...
            # 注: 这里打印的是【请求】batch; 实时源(摄像头/RTSP)会在 pipeline 内被强制成 1,
            #     真正生效的 batch 由 ThreadedPipeline 启动时再打一条权威日志.
            logger.info(f"执行引擎: 多线程流水线 (请求 batch={batch_size}, 显示与主循环解耦)")
