...
# YAML 用户友好名 → DrawStyle 内部字段名 的映射.
# YAML 面向最终用户(box_thickness / text_scale 更直觉),
# DrawStyle 是程序内部模型(line_width / font_size).
# 这个映射让两端各说各的, 互不污染.
...
# YAML 里写了但 DrawStyle 还没支持的字段, 读配置时 warn 跳过, 不崩.
_UNSUPPORTED_STYLE_KEYS: set[str] = {
    "text_bg_alpha",
}


def _translate_style_overrides(raw_style: dict[str, Any]) -> dict[str, Any]:
    """把 YAML 用户友好 key 翻译成 DrawStyle 能吃的 key, 不支持的 warn 跳过."""
    translated: dict[str, Any] = {}
    for key, value in (raw_style or {}).items():
        if key in _UNSUPPORTED_STYLE_KEYS:
            logger.warning(
                "infer_pipeline.yaml style.%s 暂未支持, 已跳过 (后续版本会实现).",
                key,
            )
            continue
        mapped = _STYLE_KEY_MAP.get(key, key)
        if mapped != key:
            logger.debug("style key 映射: %s → %s", key, mapped)
...
def load_pipeline_config(yaml_path: str | Path | None = None) -> PipelineConfig:
    ...
    return PipelineConfig(
        ...
        style_overrides=_translate_style_overrides(vz.get("style") or {}),
        source_path=path,
    )
